    ...
    <main v-if="article" class="relative">
      ...
      <div class="relative h-[70vh] overflow-hidden">
        ...
        <div class="absolute inset-x-0 bottom-0 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
          ...
          <h1 class="text-4xl sm:text-5xl lg:text-6xl font-bold text-white mb-8 [text-wrap:balance] animate-slide-up">
            {{ localizedTitle }}
          </h1>
          ...
          <div class="flex items-center gap-6 text-white animate-slide-up">
            <div class="flex items-center gap-3">
              <div class="relative">
                ...
                />
                <div class="absolute -bottom-1 -right-1 w-5 h-5 bg-green-500 rounded-full border-2 border-white dark:border-gray-900" />
              </div>
              <div>
                <div class="font-medium">{{ article.author.name }}</div>
                <div class="text-sm text-gray-300">{{ localizedAuthorBio }}</div>
              </div>
            </div>
      ...
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 -mt-32 relative z-10">
        <article class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden animate-scale-up">
          <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between bg-gray-50 dark:bg-gray-800/50">
            ...
            <div class="flex items-center gap-3">
              ...
              <button
                @click="shareArticle"
                class="flex items-center gap-2 px-4 py-2 rounded-full bg-primary-600 text-white hover:bg-primary-700 transition-all duration-300 hover:scale-105"
              >
        ...
        <section class="mt-12 bg-white dark:bg-gray-800 rounded-2xl p-6 sm:p-8">
          <div class="flex items-center justify-between mb-8">
            ...
            <div class="flex items-center gap-2">
              <span class="text-gray-600 dark:text-gray-400">{{ t('article.commentSection.sortBy') }}</span>
              <select
                v-model="sortBy"
                class="bg-gray-100 dark:bg-gray-700 rounded-lg border-0 py-1.5 pl-3 pr-8 focus:ring-2 focus:ring-primary-500"
              >
          ...
          <div class="mb-8 bg-gray-50 dark:bg-gray-700/50 rounded-xl p-6">
            <div class="mb-4 space-y-4">
              <input
                v-model="authorName"
                type="text"
                :placeholder="t('article.commentSection.yourName')"
                class="w-full bg-white dark:bg-gray-700 rounded-lg border-0 py-3 px-4 focus:ring-2 focus:ring-primary-500"
                maxlength="50"
              />
              <textarea
                v-model="newComment"
                rows="4"
                :placeholder="t('article.commentSection.joinConversation')"
                class="w-full bg-white dark:bg-gray-700 rounded-lg border-0 py-3 px-4 focus:ring-2 focus:ring-primary-500"
                maxlength="1000"
              />
            </div>
...
const galleryImages = computed(() => {
  ...
  return localizedImages.value.map((image, index) => ({
    url: image.url,
    alt: `${localizedTitle.value} - Image ${index + 1}`,
